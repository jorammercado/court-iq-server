...
const createPlayer = async (player) => {
  try {
    const { player, birth_year, age, season, experience, pos, tm } = player``
    const newPlayer = await db.one(
      `INSERT INTO players(player, birth_year, age, season,` +
        ` experience, pos, tm)` +
        ` VALUES($1, $2, $3, $4, $5, $6, $7) RETURNING *`,
      [player, birth_year, age, season, experience, pos, tm]
    )
    return newPlayer
  } catch (err) {
...
const updatePlayer = async (player_id, player) => {
  try {
    ...
    const updatedPlayer = await db.one(
      ...
      [player, birth_year, age, season, experience, pos, tm, player_id]
    )
