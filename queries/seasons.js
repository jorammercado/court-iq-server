...
const getAllSeasons = async (player_id) => {
  try {
    const allSeasons = await db.any(`SELECT * FROM seasons WHERE player_id=$1`, player_id)
    return allSeasons
  } catch (err) {
...
const deleteSeason = async (season_id) => {
  try {
    const deletedSeason = await db.one(
      `DELETE FROM seasons WHERE season_id=$1 RETURNING *`,
      season_id
    )
...
const createSeason = async (seasonData) => {
  try {
    ...
    const newSeason = await db.one(
      `INSERT INTO seasons(season, player,` +
