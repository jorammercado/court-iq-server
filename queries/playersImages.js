...
const deletePlayerImage = async (player) => {
  try {
    const playerImage = await db.any(`SELECT * FROM playersimage WHERE player=$1`, player)
...
const createPlayerImage = async (playerimage) => {
  try {
    ...
    const newPlayerImage = await db.one(
      `INSERT INTO playersimage(player, birth_date, image_url)` + ` VALUES($1, $2, $3) RETURNING *`,
...
const updatePlayerImage = async (playerImageData) => {
  try {
    const playerImage = await db.any(
      `SELECT * FROM playersimage WHERE player=$1`,
