...
const getOneUserByUserName = async ({ username }) => {
  try {
    const oneUser = await db.oneOrNone('SELECT * FROM users WHERE username=$1', username)
    return oneUser
  } catch (err) {
...
const deleteUserByUsername = async (username) => {
  try {
    const deletedUser = await db.one(`DELETE FROM users WHERE username=$1 RETURNING *`, username)
    return deletedUser
  } catch (err) {
...
const deleteUser = async (user_id) => {
  try {
    const deletedUser = await db.one(`DELETE FROM users WHERE user_id=$1 RETURNING *`, user_id)
    return deletedUser
  } catch (err) {
    return { err: `${err}, sql query error in deleting a user` }
  }
