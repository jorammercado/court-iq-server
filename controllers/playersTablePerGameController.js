...
const { checkPlayers, checkPlayerIndex, checkPlayerName } = require('../validations/checkPlayers')
    ...
    if (req.query.order) {
      allPlayers.sort((a, b) => {
        if (req.query.order === 'asc' || req.query.order === 'desc') {
          ...
          else if (a.player.toLowerCase() > b.player.toLowerCase()) return 1
          else return 0
        } else if (req.query.order === 'asc_birthyear' || req.query.order === 'desc_birthyear') {
          if (Number(a.birth_year) < Number(b.birth_year)) return -1
          else if (Number(a.birth_year) > Number(b.birth_year)) return 1
          else return 0
      ...
      if (
        req.query.order === 'asc' ||
        req.query.order === 'asc_birthyear' ||
        req.query.order === 'asc_age' ||
        req.query.order === 'asc_season' ||
        req.query.order === 'asc_experience' ||
        req.query.order === 'asc_pos' ||
        req.query.order === 'asc_tm'
      )
        res.status(200).json(allPlayers)
      else if (
        req.query.order === 'desc' ||
        req.query.order === 'desc_birthyear' ||
        req.query.order === 'desc_age' ||
        req.query.order === 'desc_season' ||
        req.query.order === 'desc_experience' ||
        req.query.order === 'desc_pos' ||
        req.query.order === 'desc_tm'
      )
        res.status(200).json(allPlayers.reverse())
      else res.status(400).json({ error: `Order query error in index path.` })
    } else res.status(200).json(allPlayers)
  } catch (error) {
...
players.delete('/:id', checkPlayerIndex, async (req, res) => {
  try {
    ...
    if (deletedPlayer) res.status(200).json({ success: true, deletedPlayer })
...
players.put('/:id', checkPlayerName, checkPlayerIndex, async (req, res) => {
  try {
    const { player_id } = req.params
    const player = req.body
