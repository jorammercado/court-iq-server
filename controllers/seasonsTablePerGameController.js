...
seasons.get('/', checkSeasons, checkPlayerIndex, async (req, res) => {
  try {
    ...
    if (req.query.order) {
      allSeasons.sort((a, b) => {
        if (req.query.order === 'asc' || req.query.order === 'desc') {
          if (Number(a.season) < Number(b.season)) return -1
          else if (Number(a.season) > Number(b.season)) return 1
          else return 0
        } else if (req.query.order === 'asc_pos' || req.query.order === 'desc_pos') {
          if (a.pos.toLowerCase() < b.pos.toLowerCase()) return -1
          else if (a.pos.toLowerCase() > b.pos.toLowerCase()) return 1
          else return 0
        } else if (req.query.order === 'asc_age' || req.query.order === 'desc_age') {
          if (Number(a.age) < Number(b.age)) return -1
          else if (Number(a.age) > Number(b.age)) return 1
          else return 0
        }
        if (req.query.order === 'asc_experience' || req.query.order === 'desc_experience') {
          if (Number(a.experience) < Number(b.experience)) return -1
          else if (Number(a.experience) > Number(b.experience)) return 1
          else return 0
        } else if (req.query.order === 'asc_tm' || req.query.order === 'desc_tm') {
          if (a.tm.toLowerCase() < b.tm.toLowerCase()) return -1
          else if (a.tm.toLowerCase() > b.tm.toLowerCase()) return 1
          else return 0
        }
        ...
        if (req.query.order === 'asc_x3p_percent' || req.query.order === 'desc_x3p_percent') {
          ...
          req.query.order === 'asc_x2p_per_game' ||
          req.query.order === 'desc_x2p_per_game'
        ) {
          if (Number(a.age) < Number(b.age)) return -1
          else if (Number(a.age) > Number(b.age)) return 1
          else return 0
        } else if (
          req.query.order === 'asc_x2pa_per_game' ||
          req.query.order === 'desc_x2pa_per_game'
        ) {
          if (Number(a.age) < Number(b.age)) return -1
          else if (Number(a.age) > Number(b.age)) return 1
          else return 0
        } else if (
          req.query.order === 'asc_x2p_percent' ||
          req.query.order === 'desc_x2p_percent'
        ) {
          if (Number(a.age) < Number(b.age)) return -1
          else if (Number(a.age) > Number(b.age)) return 1
          else return 0
        } else if (
          req.query.order === 'asc_e_fg_percent' ||
          req.query.order === 'desc_e_fg_percent'
        ) {
          if (Number(a.age) < Number(b.age)) return -1
          else if (Number(a.age) > Number(b.age)) return 1
          else return 0
        } else if (
          req.query.order === 'asc_ft_per_game' ||
          req.query.order === 'desc_ft_per_game'
        ) {
          if (Number(a.age) < Number(b.age)) return -1
          else if (Number(a.age) > Number(b.age)) return 1
          else return 0
        } else if (
          req.query.order === 'asc_fta_per_game' ||
          req.query.order === 'desc_fta_per_game'
        ) {
          if (Number(a.age) < Number(b.age)) return -1
          else if (Number(a.age) > Number(b.age)) return 1
          else return 0
          ...
          req.query.order === 'asc_orb_per_game' ||
          req.query.order === 'desc_orb_per_game'
        ) {
          if (Number(a.age) < Number(b.age)) return -1
          else if (Number(a.age) > Number(b.age)) return 1
          else return 0
        } else if (
          req.query.order === 'asc_drb_per_game' ||
          req.query.order === 'desc_drb_per_game'
        ) {
          if (Number(a.age) < Number(b.age)) return -1
          else if (Number(a.age) > Number(b.age)) return 1
          else return 0
          ...
          req.query.order === 'asc_trb_per_game' ||
          req.query.order === 'desc_trb_per_game'
        ) {
          if (Number(a.age) < Number(b.age)) return -1
          else if (Number(a.age) > Number(b.age)) return 1
          else return 0
        } else if (
          req.query.order === 'asc_ast_per_game' ||
          req.query.order === 'desc_ast_per_game'
        ) {
          if (Number(a.age) < Number(b.age)) return -1
          else if (Number(a.age) > Number(b.age)) return 1
          else return 0
        } else if (
          req.query.order === 'asc_stl_per_game' ||
          req.query.order === 'desc_stl_per_game'
        ) {
          if (Number(a.age) < Number(b.age)) return -1
          else if (Number(a.age) > Number(b.age)) return 1
          else return 0
        } else if (
          req.query.order === 'asc_blk_per_game' ||
          req.query.order === 'desc_blk_per_game'
        ) {
          if (Number(a.age) < Number(b.age)) return -1
          else if (Number(a.age) > Number(b.age)) return 1
          else return 0
        } else if (
          req.query.order === 'asc_tov_per_game' ||
          req.query.order === 'desc_tov_per_game'
        ) {
          if (Number(a.age) < Number(b.age)) return -1
          else if (Number(a.age) > Number(b.age)) return 1
          else return 0
        } else if (
          req.query.order === 'asc_pf_per_game' ||
          req.query.order === 'desc_pf_per_game'
        ) {
          if (Number(a.age) < Number(b.age)) return -1
          else if (Number(a.age) > Number(b.age)) return 1
          else return 0
        } else if (
          req.query.order === 'asc_pts_per_game' ||
          req.query.order === 'desc_pts_per_game'
        ) {
          if (Number(a.age) < Number(b.age)) return -1
          else if (Number(a.age) > Number(b.age)) return 1
          else return 0
      ...
      if (orderUp.includes(req.query.order)) res.status(200).json({ ...player, allSeasons })
      else if (orderDown.includes(req.query.order)) {
        allSeasons = allSeasons.reverse()
        res.status(200).json({ ...player, allSeasons })
      } else
        res.status(400).json({
          error: `Order query error` + ` in index path for seasons.`,
        })
    } else {
      res.status(200).json({ ...player, allSeasons })
    }
    ...
    res.status(400).json({
      error: `${error}, error in index` + ` controller path for seasons.`,
    })
  }
})
...
seasons.get('/:season_id', checkPlayerIndex, checkSeasonIndex, async (req, res) => {
  try {
    ...
    res.status(400).json({
      error: `${error}, error in show` + ` controller path for artworks`,
    })
  }
})
...
seasons.delete('/:season_id', checkPlayerIndex, checkSeasonIndex, async (req, res) => {
  try {
    ...
    res.status(400).json({
      error: `${error}, error in delete` + ` controller path for seasons`,
    })
  }
})

seasons.post('/', checkPlayerIndex, checkSeasonName, async (req, res) => {
  try {
    const { player_id } = req.params
    const player = await getOnePlayer(player_id)
    ...
    res.status(400).json({
      error: `${error}, error in new` + ` controller path for seasons`,
    })
  }
})
...
seasons.put(
  ...
  async (req, res) => {
    try {
      ...
      res.status(400).json({
        error: `${error}, error in update` + ` controller path for seasons`,
      })
    }
  }
)
