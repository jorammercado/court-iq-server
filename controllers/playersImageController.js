...
playerimage.delete('/:player', async (req, res) => {
  try {
    ...
    if (deletedPlayerImage) res.status(200).json({ success: true, deletedPlayerImage })
    else res.status(404).json({ error: `PlayerImage not found.` })
  } catch (error) {
...
playerimage.put('/:player', checkPlayerImageNameExists, checkPlayerImageName, async (req, res) => {
  try {
