const { getAllUsers, getOneUserByUserName, getOneUserByEmail } = require('../queries/users')
...
const checkUsernameExistsOtherThanSelf = async (req, res, next) => {
  ...
  if (registeredUserByUserName.user_id === Number(user_id) || !registeredUserByUserName)
    return next()
  else res.status(400).json({ error: 'user already registered with this username' })
}
...
const checkEmailExistsOtherThanSelf = async (req, res, next) => {
  const { user_id } = req.params
  const registeredUserByEmail = await getOneUserByEmail(req.body)
  if (registeredUserByEmail.user_id === Number(user_id) || !registeredUserByEmail) next()
  else res.status(400).json({ error: 'user already registered with this username' })
}
...
const checkValidUsername = async (req, res, next) => {
  ...
  if (allUsernames.includes(Number(username))) return next()
  else
    res.status(400).json({
...
const checkUserIndex = async (req, res, next) => {
  ...
  if (ids.includes(Number(user_id))) return next()
  else
    res.status(400).json({
