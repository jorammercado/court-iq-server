...
const checkSeasons = async (req, res, next) => {
  ...
  if (allSeasons[0]) {
    ...
    res.status(500).json({
      error: `server error in getAllSeasons,` + ` list validation failed, no seasons`,
    })
  }
}
...
const checkPlayerIndex = async (req, res, next) => {
  ...
  if (ids.includes(Number(player_id))) return next()
  else
    res.status(400).json({
      error: `server error in checkPlayerIndex,` + ` id validation failed`,
    })
}
...
const checkSeasonIndex = async (req, res, next) => {
  ...
  if (ids.includes(Number(season_id))) return next()
  else
    res.status(400).json({
      error: `server error in checkSeasonIndex,` + ` id validation failed for seasons`,
    })
}
...
const checkSeasonName = (req, res, next) => {
  if (req.body.season) {
    ...
    res.status(400).json({
      error: `season year is required,` + ` name validation failed for season data`,
    })
  }
}
